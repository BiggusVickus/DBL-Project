...
#Read the csv file
df = pd.read_csv('DBL-Project/static/Visualizations/Uploads/fixation_data.csv', parse_dates=[0])
df = df_paths.astype({'Timestamp': int, 'StimuliName': str, 'FixationIndex': float, 'FixationDuration': float, 'MappedFixationPointX': int, 'MappedFixationPointY' : int, 'user': str, 'description': str})

stimulus = '02_Berlin_S1.jpg'
...
for index, row in df_copy.iterrows(): #Iterates every row one by one
    ...
    for index2, OtherRow in df_copy.iterrows(): #Iterates every row. 
        if index != index2: #If the row found is not the same row, the distance between the two points is calculated.
            #closeness_val_2 = int(row['closeness'])
            ...
            #if ((xi2_val - xi_val)<close) && ((yi2_val - yi_val)<15) :
                closeness_val = closeness_val + 1 
                #closeness_val_2 = closeness_val_2 + 1
...
p = figure(
    ...
    x_range=(0,1500), 
    y_range=(0,1500),
    title = 'Example (02_Berlin_S1.jpg)',
    x_axis_label = 'x-axis',
    y_axis_label = 'y-axis'
    )
# 1650 x 1200
# 11:8
p.image_url(url = ['images/02_Berlin_S1.jpg'], x = 0 , y = 1500, w = 1500 , h = 1500)
