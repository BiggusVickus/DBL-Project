...
def index_vis3():
	...
	vis_text = 'The idea behind this visualization is that the user can see a heatmap of a specific user, map, and map color. It helps  the user understand the density of where the majority of the data is using a fun interactive colorcoding. All fixation points from each user fo a specific stimuli are added to the map. Depending on how many other dots are close to a dot, the dot\'s color changes from blue to red. The denser the dots, the redder the dots will appear. The user can select the image they want to analyse as well as change 1 values \(p\). The closeness of a dot is calculated by taking a dot on the screen and making a virtual circle around it. If another dot is in that circle, the closeness value is increased by 1 for both dots. Repeat for all other dots. Then the dots are put onto the graph with a color coding that has been evenly distributed so that a certain percentage of dots have a color coding. The radius of the circle is dynamically changed with the input size of the width and height of the image, the proportion of the image size constant. The user can select the \(0 < p \leq 1\). The  formula for the radius of the circle with respect to area of the image and a proportion constant is \( \sqrt{w*h*p \over \pi} \), where \(w=width, h=height, p=\) \({area \; of\; circle}\over{area\; of\; image}\). If \(p=0.05\), then the area of the circle is \(5\%\) of the area of the image.'
	return render_template('vispage.html', vis_page = vis_page, vis_text=vis_text)
